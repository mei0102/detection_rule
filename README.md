...
各ルールは「検知概要・検知意図・過検知予想」から読み始められます。既存50件の各ルールは「検知概要・検知意図・過検知予想」から読み始められます。既存50件の現行SPLは変更前のリポジトリ版（5d42ac6）を折りたたみ表示しています。既存分の既存分の提案SPLは検知条件を維持し、ES用の出力列と調査優先度を整えた版です。新規分は現行SPLなしと記載しています。新規分は現行SPLなしと記載しています。Risk Analysis・Notableのアクション設定や顧客環境への適用は行っていません。
