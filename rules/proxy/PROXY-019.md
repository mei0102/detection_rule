# PROXY-019 URLにシェルの連結記号とコマンドを含む要求

**検知概要：** URLの中にシェルの連結記号と、whoamiやcurlなどのコマンド名が並ぶ要求を探す。

**検知意図：** Web入力を通してサーバーにOSコマンドを実行させようとする要求を調べる。

**過検知予想：** コマンド例の検索、開発ツール、許可された診断。利用目的の裏付けが必要で、単独で悪性とは判断しない。

|判断の目安|内容|
|---|---|
|**重要度**|**P2：通常調査**（感染確定度ではない）|
|重要度の理由|不審な操作・条件を絞っているが、コマンド例の検索、開発ツール、許可された診断でも成立し、追加確認が必要。|
|ルールの役割|単独イベント・集約／調査候補|
|発報したら|同一主体の前後操作を確認し、不正な後続操作や重要資産への影響があればP1へ引き上げる。|
|疑いを強める追加証拠|元URLの文脈、対象アプリ、サーバーの子プロセス、診断計画。 正常な説明がつかず、同一主体の不正操作が裏付けられること。|
...
- 共通する実装パターン：URLの中にシェルの連結記号と、whoamiやcurlなどのコマンド名が並ぶ要求を探す。
- 長期利用できる攻撃行動：Web入力を通してサーバーにOSコマンドを実行させようとする要求を調べる。
- 今回採用する条件と理由：15分に3要求・2種類以上、連結記号の後に掲載コマンドがある。 単発の一致や通常通信と区別するための設計値。
- 採用しない条件と理由：未取得の本文・端末情報や許可台帳を必須にすると、このProxyだけで成立しないため。別の調査で補う。

## 検知意図と限界

- 検知できること：URLの中にシェルの連結記号と、whoamiやcurlなどのコマンド名が並ぶ要求を探す。
- 検知できないこと：URL以外の本文・ヘッダーは未評価。通常の&区切りパラメータでも一致し得る。実行成功はサーバー側で確認。
- 意図的に検知しないこと：閾値未満・掲載していない表現の通信、Proxyを通らない通信。
- 想定される回避方法：要求を時間・宛先へ分散する、文字列を変更する、観測対象外の本文やプロトコルへ移す。

## 誤検知

- 主な正常動作：コマンド例の検索、開発ツール、許可された診断。
- 誤検知を識別する追加情報：元URLの文脈、対象アプリ、サーバーの子プロセス、診断計画。
...
| where match(decoded_url, "[;|&]s*(whoami|id|uname|curl|wget|powershell|cmd)(s|$|[;&|])")
| stats count as requests
    ...
    by src_ip url_host
| where requests >= 3 AND distinct_urls >= 2
| eval rule_id="PROXY-019", attack_id="T1190",
       risk_object=src_ip, risk_object_type="system", risk_score=40, severity="medium"
```

- 差分：新規。URLにシェルの連結記号とコマンドを含む要求を調査対象として追加。
...
- notable title：PROXY-019 URLにシェルの連結記号とコマンドを含む要求（採用時の案）。
- drilldown：src_ipと結果の宛先で同じ15分窓の元ログを再検索。元URLの文脈、対象アプリ、サーバーの子プロセス、診断計画を確認。

## 閾値

- 初期値：15分に3要求・2種類以上、連結記号の後に掲載コマンドがある。
- 根拠：動作の比較に使う仮の設計値。参考資料の推奨値や実環境の正常分布ではない。
- 高感度設定：1要求。その他の条件は標準を維持する案。
- 標準設定：提案SPLの条件。
- 低誤検知設定：5要求・3種類。追加情報が必要なら収集を確認。効果は未検証。
- ベースライン取得方法：30日を同じ15分窓と集約軸で集計し、コマンド例の検索、開発ツール、許可された診断の確認済み結果と比較する。

## 人が評価すべき点

- 採用判断に必要な確認事項：必要列・HTTPS可視性・社内要求元の識別、正常業務との衝突。
- 不足データ：実ログ、必要列の充足率、正常業務台帳、TP/FP判定、実行時間、ESバージョン。
- 最大のリスク：コマンド例の検索、開発ツール、許可された診断を攻撃と誤認すること。
- 推奨判断：調査サーチ。
- 推奨理由：Web入力を通してサーバーにOSコマンドを実行させようとする要求を調べる。 追加証拠を確認してからNotable/RBA化を判断する。

## 参考文献

- [T1190 Exploit Public-Facing Application](https://attack.mitre.org/techniques/T1190)：対応する攻撃行動を参照。下記の条件・閾値は独自の調査案。
- [OWASP — OS Command Injection Defense](https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html)：シェル解釈と連結記号によるコマンド注入の性質を参照。
