...
**検知概要：** BITSを名乗るクライアントが、ドメイン名ではなくIPv4アドレスを指定してファイルを取得する動きを探す。

**検知意図：** 標準の転送機能で外部IPからファイルを取り込む動きを調べる。

**過検知予想：** 直接IPのソフト配布。 悪性かどうかは、利用目的や実行主体の確認が必要。
...
|疑いを強める追加証拠|BITSジョブ、保存先、IPの所有者。 正常な説明がつかず、同一主体の不正操作が裏付けられること。|
...
- 脅威固有の指標：BITS UAとIPv4直指定URL。
- 共通する実装パターン：名前解決を介さず転送ジョブで取得。
- 長期利用できる攻撃行動：BITSによる外部IP直指定ダウンロード。特定攻撃者への帰属は行わない。
- 今回採用する条件と理由：BITSのUser-Agentで外部IPv4直指定URLを取得し、1KiB以上を受信。 この観測範囲で名前解決を介さず転送ジョブで取得の候補を抽出するため。
- 採用しない条件と理由：DNS名・IPv6指定、小容量取得は今回の条件外。別の観測・検知条件が必要なため、この検索に混在させない。未提供の資産台帳による除外も行わない。

## 検知意図と限界

- 検知できること：BITSのUser-Agentで外部IPv4直指定URLを取得し、1KiB以上を受信。
- 検知できないこと：IPv4直指定だけが対象。正規配布も一致し、取得物の実行は不明。
- 意図的に検知しないこと：DNS名・IPv6指定、小容量取得。
- 想定される回避方法：上記の条件外の経路・表記・タイミングへ変更する、または必要ログの記録を妨げる。

## 誤検知

- 主な正常動作：直接IPのソフト配布。
- 誤検知を識別する追加情報：BITSジョブ、保存先、IPの所有者。
- 改善方法：保存先や取得後の実行を照合。追加情報が取得できる場合の案で、今回のSPLには未適用。
- 除外が必要な場合の対象と根拠：直接IPのソフト配布のうち、上記情報と承認記録で一致を確認した主体・対象・時間帯だけ。現時点の除外はなし。
...
- index、sourcetype、製品固有フィールドの置換箇所：`index=detection_lab`は仮置き。実indexと確認済みsourcetypeへ置換し、`log_type`は共通分類を用意するか実ログ条件へ置換。src_ip←元クライアント、url←HTTP URL、http_method/status←要求/応答、bytes_out/in←要求単位の送受信量。必要列：`_time`, `log_type`, `src_ip`, `url`, `user_agent`, `http_method`, `http_status`, `bytes_in`, `dest_zone`。
...
- notable title：`PROXY-003 BITSによる外部IP直指定ダウンロード`。Notable化を採用する場合の案。
- drilldown：結果の`src_ip`を元ログの同じ列へ渡し、検知対象窓の前後5分を再検索。BITSジョブ、保存先、IPの所有者を表示・照合。未取得の列は追加収集が必要。

## 閾値

- 初期値：BITSのUser-Agentで外部IPv4直指定URLを取得し、1KiB以上を受信。 数値のないイベント・文字列条件は1件一致で結果候補。
- 根拠：上記行動の候補を切り出す設計値。参照ルールの推奨値や実環境の正常分布を証明する値ではない。
- 高感度設定：受信256B。記載以外の条件は標準と同じ。SPLには未適用。
- 標準設定：提案SPLの条件。初期値と同一。
- 低誤検知設定：受信10KiB（UA・IPv4条件維持）。追加情報が必要な案は取得確認が前提で、効果は未確認。
- ベースライン取得方法：30日を同じ検索窓・集約主体`src_ip`で区切り、正常と確認した直接IPのソフト配布とそれ以外を分け、件数・対象数・数値条件の分布を比較。存在しない統計は補完しない。

## 人が評価すべき点

- 採用判断に必要な確認事項：HTTPSのURL可視性・HTTP項目とリクエスト単位バイトの取得状況。BITSジョブ、保存先、IPの所有者で正常業務と識別できるか。
- 不足データ：実index/sourcetype、必要列の充足率、ESバージョン、発報数・TP/FP判定、資産・業務台帳、実行時間。
- 最大のリスク：直接IPのソフト配布との衝突と、DNS名・IPv6指定、小容量取得の取りこぼし。
- 推奨判断：調査サーチ（補助シグナル）。
