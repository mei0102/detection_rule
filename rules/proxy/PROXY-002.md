...
**検知概要：** PowerShellを名乗るクライアントがWeb取得に成功した記録を探す。

**検知意図：** スクリプトからの不審なダウンロードを調べる。UAだけでは実行プログラムを確定できない。

**過検知予想：** 管理スクリプト、API取得。 悪性かどうかは、利用目的や実行主体の確認が必要。
...
|疑いを強める追加証拠|端末の4104/4688、URL、ジョブ所有者。 正常な説明がつかず、同一主体の不正操作が裏付けられること。|
