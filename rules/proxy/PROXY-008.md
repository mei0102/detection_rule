# PROXY-008 同じURLへのほぼ一定間隔の通信

**検知概要：** 同じ端末が同じURLへ、ほぼ時計どおりにGET通信を繰り返す動きを探す。例は約60秒ごとの通信。

**検知意図：** マルウェアが外部サーバーへ定期的に指示を取りに行く通信を探す。周期性だけで感染とは判断しない。

**過検知予想：** ヘルスチェック、自動更新。 悪性かどうかは、利用目的や実行主体の確認が必要。

判定の目安は30分内に12間隔以上（通常13回以上の通信）、平均30〜300秒、間隔のばらつき（標準偏差）が平均の10%以下。平均60秒なら標準偏差6秒以下で、各間隔が必ず54〜66秒という意味ではありません。
...
|疑いを強める追加証拠|URL用途、レスポンス量、送信元プロセス。 正常な説明がつかず、同一主体の不正操作が裏付けられること。|
...
- 脅威固有の指標：URL単位の通信間隔。固定IOCなし。
- 共通する実装パターン：定間隔のHTTPポーリング。
- 長期利用できる攻撃行動：同じURLへのほぼ一定間隔の通信。特定攻撃者への帰属は行わない。
- 今回採用する条件と理由：同じ送信元・URLへのGET間隔が30〜300秒で、揺らぎが平均の10%以下。 この観測範囲で定間隔のHTTPポーリングの候補を抽出するため。
- 採用しない条件と理由：URL変更、大きなジッター、周期範囲外は今回の条件外。別の観測・検知条件が必要なため、この検索に混在させない。未提供の資産台帳による除外も行わない。

## 検知意図と限界

- 検知できること：同じ送信元・URLへのGET間隔が30〜300秒で、揺らぎが平均の10%以下。
- 検知できないこと：ヘルスチェックでも一致。URLを毎回変更する通信や大きなジッターは対象外。
- 意図的に検知しないこと：URL変更、大きなジッター、周期範囲外。
- 想定される回避方法：上記の条件外の経路・表記・タイミングへ変更する、または必要ログの記録を妨げる。

## 誤検知

- 主な正常動作：ヘルスチェック、自動更新。
- 誤検知を識別する追加情報：URL用途、レスポンス量、送信元プロセス。
- 改善方法：監視ジョブと間隔・応答サイズを照合。追加情報が取得できる場合の案で、今回のSPLには未適用。
- 除外が必要な場合の対象と根拠：ヘルスチェック、自動更新のうち、上記情報と承認記録で一致を確認した主体・対象・時間帯だけ。現時点の除外はなし。
...
- index、sourcetype、製品固有フィールドの置換箇所：`index=detection_lab`は仮置き。実indexと確認済みsourcetypeへ置換し、`log_type`は共通分類を用意するか実ログ条件へ置換。src_ip←元クライアント、url←HTTP URL、http_method/status←要求/応答、bytes_out/in←要求単位の送受信量。必要列：`_time`, `log_type`, `src_ip`, `url`, `http_method`, `http_status`。
...
- notable title：`PROXY-008 同じURLへのほぼ一定間隔の通信`。Notable化を採用する場合の案。
- drilldown：結果の`src_ip`を元ログの同じ列へ渡し、検知対象窓の前後5分を再検索。URL用途、レスポンス量、送信元プロセスを表示・照合。未取得の列は追加収集が必要。

## 閾値

- 初期値：同じ送信元・URLへのGET間隔が30〜300秒で、揺らぎが平均の10%以下。 数値のないイベント・文字列条件は1件一致で結果候補。
...
- ベースライン取得方法：30日を同じ検索窓・集約主体`src_ip`で区切り、正常と確認したヘルスチェック、自動更新とそれ以外を分け、件数・対象数・数値条件の分布を比較。存在しない統計は補完しない。

## 人が評価すべき点

- 採用判断に必要な確認事項：HTTPSのURL可視性・HTTP項目とリクエスト単位バイトの取得状況。URL用途、レスポンス量、送信元プロセスで正常業務と識別できるか。
- 不足データ：実index/sourcetype、必要列の充足率、ESバージョン、発報数・TP/FP判定、資産・業務台帳、実行時間。
- 最大のリスク：ヘルスチェック、自動更新との衝突と、URL変更、大きなジッター、周期範囲外の取りこぼし。
- 推奨判断：調査サーチ（補助シグナル）。
